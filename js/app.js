...
const comment = (() => {
    ...
    const send = async () => {
        ...
        if (nama.length >= 35) {
            alert('Full name extension maximum 35 characters');
            return;
        }

        if (hadir == 0) {
            alert('Please choose attendance');
            return;
        }
    ...
    const innerComment = (data) => {
        ...
        <div class="d-flex flex-wrap justify-content-between align-items-center">
            <div class="d-flex flex-wrap justify-content-start align-items-center">
                ...
                ${owns.has(data.uuid)
                ? `
                <button style="font-size: 0.8rem;" onclick="comment.edit(this)" data-uuid="${data.uuid}" class="btn btn-sm btn-outline-dark rounded-3 py-0 ms-1">Change</button>
                <button style="font-size: 0.8rem;" onclick="comment.hapus(this)" data-uuid="${data.uuid}" class="btn btn-sm btn-outline-dark rounded-3 py-0 ms-1">Delete</button>`
                : ''}
            </div>
            ...
            .then((res) => {
                if (res.code == 200) {
                    ...
                    if (res.data.length == 0) {
                        UCAPAN.innerHTML = `<div class="h6 text-center">No data available.</div>`;
                    }
                }
            })
